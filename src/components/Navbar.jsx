...
export default function Navbar() {
  ...
  return (
    <div className='flex items-center px-2 border-solid border-2 rounded h-14 ml-2 mr-2 mb-2 dark:bg-slate-600 dark:border-slate-400'>
        ...
        {user && <InitialsAvatar name={user.username} className='selected rounded-full py-1 px-1 uppercase text-xs font-bold cursor-pointer tracking-wider text-blue-500 border-solid border-2 border-blue-500' /> }
        <NavLink className={(element) => element.isActive ? 'selected' : ''} to="/shoes">{<AiOutlineShopping className='w-6 h-6 dark:bg-slate-400'/>}</NavLink>
        ...
        {isLoggedIn && <NavLink to='/shoppingcar'><AiOutlineShoppingCart className='w-6 h-6 dark:bg-slate-400'/></NavLink>}
        {isLoggedIn && <button onClick={() => logOutUser()}>{<GiExitDoor className='w-6 h-6 dark:bg-slate-400'/>}</button>}
        <Switcher />
        <button onClick={() => navigate(-1)}>{<AiOutlineRollback className='w-6 h-6 dark:bg-slate-400'/>}</button>
      </nav>
    </div>
  )
}
